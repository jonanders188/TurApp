...
export function TrailCard({ trail, featured = false }: { trail: Trail; featured?: boolean }) {
  ...
  return (
    <Link
      href={`/turer/${trail.slug}`}
      className={`group block overflow-hidden rounded-[2rem] border bg-white shadow-sm transition hover:-translate-y-1 hover:shadow-2xl ${featured ? 'border-emerald-400 ring-4 ring-emerald-100' : 'border-emerald-950/10'}`}
    >
      ...
      <div className="p-5">
        <div className="flex flex-wrap items-center gap-2">
          ...
          {trail.curated === false ? <span className="rounded-full bg-amber-100 px-3 py-1 text-xs font-black text-amber-800">Ikke verifisert</span> : null}
        </div>
        ...
        <div className="mt-4 flex items-start justify-between gap-4">
          <div className="min-w-0">
            ...
            <h3 className="mt-1 line-clamp-2 text-2xl font-black tracking-tight text-slate-950 group-hover:text-emerald-800">{trail.name}</h3>
          </div>
        ...
        <div className="mt-4 grid grid-cols-3 gap-2 text-xs text-slate-600">
          ...
          <MiniMetric value={hasRoute ? 'Ja' : 'Nei'} label="rute" />
          <MiniMetric value={trail.curated ? 'Ja' : 'Nei'} label="verifisert" />
        </div>
